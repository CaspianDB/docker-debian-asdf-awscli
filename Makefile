...
AWSCLI_RELEASE ?= $(shell cat .tool-versions | awk '$$1 == "awscli" { print $$2 }')
...
push: ## Publish to container registry.
	...
	docker tag $(IMAGE_NAME) $(DOCKER_REPO):awscli-$(AWSCLI_RELEASE:v%=%)
	docker push $(DOCKER_REPO):awscli-$(AWSCLI_RELEASE:v%=%)
